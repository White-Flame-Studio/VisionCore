...
public final class SoundCloudMediaSource implements MediaSource {
    ...
    public SoundCloudMediaSource(MediaLogger logger) {
        this.logger = logger;
    }

    @Override
    public String id() {
        return "soundcloud-ytdlp";
    }
    ...
    public ResolvedMedia resolve(URI uri) throws MediaResolveException {
        ...
        try {
            ...
            throw new MediaResolveException(uri, "Unable to find 'yt-dlp' executable in the system PATH. " + "Install it using ('pipx install yt-dlp', or through your distro package) to play SoundCloud URLs.");
        }

        StringBuilder stdout = new StringBuilder();
        StringBuilder stderr = new StringBuilder();
        Thread stdoutReader = new Thread(() -> appendAll(process.getInputStream(), stdout), "ytdlp-soundcloud-stdout");
        Thread stderrReader = new Thread(() -> appendAll(process.getErrorStream(), stderr), "ytdlp-soundcloud-stderr");
        stdoutReader.start();
        stderrReader.start();
        ...
        if (process.exitValue() != 0) {
            ...
            throw new MediaResolveException(uri, "yt-dlp couldn't resolve the URL (deleted, private, or restricted)");
        }
